...
The visual editing (hover mode) feature was completely non-functional. When the user toggled hover/edit mode from the chat panel, the Flutter app running inside the sandbox iframe never received the Pusher event because the Pusher channel subscription was skipped entirely.
...
### Root Cause 1: go_router v6 Strips URL Query Parameters
...
However, **go_router v6** calls `window.history.replaceState()` during initialization, which strips all query parameters from `window.location.search`. By the time the `useHoverWithChannel` hook runs, the search string is empty.
...
`server-utils.ts` already wrote `EXPO_PUBLIC_PROJECT_ID` to `.env.local` before starting the Metro dev server, but it did not write `EXPO_PUBLIC_SANDBOX_ID`. The sandbox ID was available as `sandbox.sandboxId` but was never persisted for the Flutter app to read.
...
The URL query param is kept as the primary source (in case go_router behavior changes), but the reliable path is the `EXPO_PUBLIC_*` env var.
...
```bash
pnpm run build:sandbox:flutter-test    # Testing template
...
1. **Template rebuild**: Run `pnpm run build:sandbox:flutter-test` to bake updated hook into image
