...
const AssistantMessageCard = memo(function AssistantMessageCard({ data, isStreaming }: { data: any; isStreaming?: boolean }) {
  ...
  if (data.subtype === 'tool_use' && data.tool_name) {
    ...
    if (toolName === 'Bash') {
      return (
        <CollapsibleCard
          cardClassName="border-y-0 border-r-0 border-l-2 border-l-orange-500/70 bg-transparent rounded-l-none shadow-none"
          ...
          streamingIndicator={isStreaming ? <Cog className="h-5 w-5 text-orange-500 animate-cog-spin" /> : undefined}
        />
      )
    }
...
const ToolResultCard = memo(function ToolResultCard({ data, isStreaming }: { data: any; isStreaming?: boolean }) {
    ...
    return (
      <CollapsibleCard
        cardClassName={isError
          ? 'border-y-0 border-r-0 border-l-2 border-l-rose-500/80 bg-rose-50/30 dark:bg-rose-950/10 rounded-l-none shadow-none'
          : 'border-y-0 border-r-0 border-l-2 border-l-emerald-500/60 bg-transparent rounded-l-none shadow-none'}
        icon={isError ? <AlertTriangle className="h-4 w-4 text-rose-500" /> : <CheckCircle className="h-4 w-4 text-emerald-500" />}
        title={<span className={isError ? 'text-rose-700 dark:text-rose-400' : 'text-muted-foreground font-normal'}>{isError ? 'Tool Error' : 'Done'}</span>}
        streamingIndicator={isStreaming ? <Cog className={`h-5 w-5 ${isError ? 'text-rose-500' : 'text-emerald-500'} animate-cog-spin`} /> : undefined}
      />
    )
  }
  ...
  return (
    <CollapsibleCard
      cardClassName={isError
        ? 'border-y-0 border-r-0 border-l-2 border-l-rose-500/80 bg-rose-50/30 dark:bg-rose-950/10 rounded-l-none shadow-none'
        : 'border-y-0 border-r-0 border-l-2 border-l-sky-500/60 bg-transparent rounded-l-none shadow-none'}
      icon={isError ? <AlertTriangle className="h-4 w-4 text-rose-500" /> : <FileText className="h-4 w-4 text-sky-500" />}
      title={<span className={isError ? 'text-rose-700 dark:text-rose-400' : 'text-muted-foreground font-normal'}>{isError ? 'Not Found' : 'File Content'}</span>}
      streamingIndicator={isStreaming ? <Cog className={`h-5 w-5 ${isError ? 'text-rose-500' : 'text-sky-500'} animate-cog-spin`} /> : undefined}
    >
