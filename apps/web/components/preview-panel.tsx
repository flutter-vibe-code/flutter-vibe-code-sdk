...
export function PreviewPanel({
  ...
  const [hasSeenFlutterModal, setHasSeenFlutterModal] = useLocalStorage('hasSeenFlutterModal', false)
  const [showFlutterModal, setShowFlutterModal] = useState(false)

  // Tunnel URL state for cloudflared
  ...
  const handleTabChange = (key: string | number) => {
    ...
    if (newTab === 'native-mobile' && !hasSeenFlutterModal) {
  ...
  // Fetch cloudflared tunnel URL when sandboxId is available
  useEffect(() => {
    if (!sandboxId || tunnelUrl || isTunnelLoading) return

    const fetchTunnel = async () => {
      setIsTunnelLoading(true)
      try {
        const response = await fetch('/api/sandbox/tunnel', {
          method: 'POST',
        ...
        console.error('[PreviewPanel] Failed to get tunnel URL:', error)
      } finally {
        setIsTunnelLoading(false)
      }
    }

    // Delay initial fetch to let server warm up
    const timeout = setTimeout(fetchTunnel, 3000)
  ...
  // Prefer live cloudflared tunnelUrl over the persisted E2B URL from DB:
  // the E2B URL becomes stale once the sandbox is recreated, while tunnelUrl
  // points to the actually running sandbox for this session.
  let basePreviewUrl = tunnelUrl || result?.url || previewUrl
    ...
    <div className={`flex flex-col h-full border-l ${viewMode === 'desktop' || viewMode === 'both' || viewMode === 'mobile-qr' ? 'w-full' : 'min-w-full max-w-full '}`}>
      ...
      <div className={` flex-1 flex flex-col bg-muted/30 ${viewMode === 'desktop' || viewMode === 'both' || viewMode === 'mobile-qr' ? 'w-full' : ''}`}>
        <div className="md:flex-1 p-0 h-full relative">
          ...
          {isLoading && (
            <div className="absolute inset-0 z-50 h-full flex min-w-full items-center justify-center bg-background/60 backdrop-blur-xl">
          ...
          {actualPreviewUrl && (
              ...
              <div className="h-full flex flex-col">
                ...
                {/* QR Code Section - Cloudflared tunnel for mobile testing */}
                {(result?.e2bUrl || tunnelUrl) && (
                  <div className="flex flex-col items-start space-y-2 max-w-xl relative -top-8">
                    ...
                    {isTunnelLoading ? (
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <Loader2 className="h-4 w-4 animate-spin" />
                        <span className="text-sm">Setting up tunnel...</span>
                      </div>
                    ) : tunnelUrl ? (
                      <div className="space-y-3">
                        <p className="text-sm text-muted-foreground">
                          Open this link on your phone's browser to test your app:
                        </p>
                        <button
                    ...
                    {(tunnelUrl || result?.e2bUrl) && (
                      <>
                        <p className="text-base pt-4">
                          <span className="font-semibold text-sm">Or scan QR Code</span>
                        </p>
                        <div className="flex items-center justify-center p-4 bg-white rounded-xl border border-gray-200">
                          <Canvas
                            text={(tunnelUrl || result?.e2bUrl)?.replace('https://', 'exp://')}
                            options={{
                        ...
                        <div className="bg-muted/50 rounded-lg p-4 space-y-3 border border-muted max-w-[270px]">
                          <p className="text-sm text-muted-foreground flex items-start gap-2">
                            ...
                            <span>Scan the QR code with your phone's camera to open in browser.</span>
                          </p>
                        </div>
                      </>
                    )}
