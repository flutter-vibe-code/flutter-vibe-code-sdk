...
export function NavHeader({
  ...
  if (isProjectPage) {
    ...
    return (
      <>
        <header className="sticky top-3 z-50 mx-3 sm:mx-4 md:mx-6 bg-background/55 supports-[backdrop-filter]:bg-background/35 backdrop-blur-2xl backdrop-saturate-150 border border-border/40 ring-1 ring-black/5 dark:ring-white/5 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.18)] rounded-2xl">
...
{/* 
            ...
            {projectTitle && (
              <div className="hidden sm:flex flex-1 justify-center">
                {isEditingTitle ? (
                  ...
                  // Project pill — title + breathing live status. Replaces the
                  // anonymous gray h2 so the IDE actually announces what
                  // project is open and whether the sandbox is alive.
                  <button
                    type="button"
                    onClick={() => projectId && setIsEditingTitle(true)}
                    title="Click to edit project title"
                    className="group inline-flex items-center gap-2.5 rounded-full border border-border/60 bg-card/50 backdrop-blur-md px-3.5 py-1.5 text-sm font-medium text-foreground/90 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] transition-all hover:border-border hover:bg-card/70 hover:shadow-[0_4px_18px_-6px_rgba(249,115,22,0.25)]"
                  >
                    {/* Status dot — always emerald + animated. Conditional
                        on sandboxId caused React #418 hydration mismatch
                        (server saw undefined, client saw populated value). */}
                    ...
                    <span className="hidden md:inline-flex h-5 items-center rounded-md bg-orange-500/10 px-1.5 text-[10px] font-semibold uppercase tracking-wider text-orange-600 dark:text-orange-400 ring-1 ring-inset ring-orange-500/20">Flutter</span>
                  </button>
                )}
              </div>
            )}
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center -space-x-1">
                ...
                {/* Publish HoverCard — primary CTA, accent fill so it stands out
                    against ghost Download/Remix on either side. */}
                <HoverCard noHover>
                  <HoverCardTrigger noHover asChild>
                    <Button
                      variant="default"
                      className="p-2 px-4 bg-gradient-to-r from-orange-500 to-amber-500 text-white hover:from-orange-600 hover:to-amber-600 shadow-[0_4px_14px_-4px_rgba(249,115,22,0.45)] ring-1 ring-orange-400/30"
  ...
  return (
    <>
      <nav className="sticky top-3 z-40 mx-3 sm:mx-4 md:mx-6 flex items-center bg-background/55 supports-[backdrop-filter]:bg-background/35 backdrop-blur-2xl backdrop-saturate-150 border border-border/40 ring-1 ring-black/5 dark:ring-white/5 shadow-[0_8px_30px_-12px_rgba(0,0,0,0.18)] rounded-2xl py-2.5 px-3 md:px-5 transition-colors">
        ...
        <div className="flex items-center gap-1 md:gap-4">
          ...
          {/* <Button variant="ghost" size="sm" asChild>
            <Link href="https://docs.fluttervibecode.dpdns.org" className="flex items-center gap-2">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => window.open('https://github.com/flutter-vibe-code/flutter-vibe-code-sdk', '_blank')}
          ...
          {session ? (
            ...
            showLogin && (
              <Button 
                ...
                className="h-8 px-3 text-xs sm:text-sm sm:h-10 sm:px-4"
              >
