...
interface UseNgrokHealthCheckOptions {
  ...
  onBackupServerReady?: (newSandboxUrl: string, newE2bUrl: string) => void // Callback when backup server starts with new URL
  onExpoError?: (errorMessage: string) => void // Callback when Expo app has a build error
  tunnelMode?: string // 'e2b-public' or 'lan'
}
...
export function useNgrokHealthCheck({
  ...
  const e2bUrlRef = useRef(e2bUrl)
  const sandboxIdRef = useRef(sandboxId)
  e2bUrlRef.current = e2bUrl
  ...
  const checkNgrokHealth = useCallback(async (): Promise<boolean> => {
    ...
    const currentE2bUrl = e2bUrlRef.current
    const currentSandboxId = sandboxIdRef.current

    if (!currentSandboxId || !currentE2bUrl || isCheckingRef.current) {
  ...
  const triggerBackupServer = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (onBackupServerReady && data.sandboxUrl && data.e2bUrl) {
          console.log('[useNgrokHealthCheck] Calling onBackupServerReady with new URLs:', data.sandboxUrl, data.e2bUrl)
          onBackupServerReady(data.sandboxUrl, data.e2bUrl)
        }
      } else {
  ...
  useEffect(() => {
    ...
    if (!enabled || !sandboxId || !e2bUrl || !serverReady || tunnelMode === 'lan') {
      if (tunnelMode === 'lan') {
        console.log('[useNgrokHealthCheck] LAN mode active, skipping ngrok health checks')
      } else if (!serverReady && enabled && sandboxId && e2bUrl) {
  ...
  }, [enabled, sandboxId, e2bUrl, serverReady, pollingInterval, checkNgrokHealth, triggerBackupServer, healthState.isStartingRecovery, tunnelMode])
