...
const FROM_EMAIL = 'Flutter Vibe Code <welcome@fluttervibecode.dpdns.org>'
const NEWSLETTER_FROM = 'Flutter Vibe Code <newsletter@fluttervibecode.dpdns.org>'
...
export async function sendWelcomeEmail(user: { name: string; email: string }) {
  ...
  const { data, error } = await getResend().emails.send({
    from: FROM_EMAIL,
    to: user.email,
    subject: 'Welcome to Flutter Vibe Code!',
