...
import { xBotReplies, xBotState, twitterLinks } from '@flutter-vibe-code/database'
...
} from '@flutter-vibe-code/payments/server'
...
const UNLINKED_USER_REPLY =
  'To build apps with Flutter Vibe Code, please create an account and link your X account at fluttervibecode.dpdns.org/settings ⚡'
...
export async function getAuthClient(): Promise<Client> {
  ...
  const oauth2Client = new auth.OAuth2User({
    ...
    callback: 'https://fluttervibecode.dpdns.org/api/x-bot/auth/callback',
...
export async function processMention(
  ...
  if (!creditCheck.canSend) {
    ...
    const upgradeText = creditCheck.usage.hasActiveSubscription
      ? `Hey @${authorUsername}! You've reached your monthly generation limit. Your quota resets on the 1st of next month. Visit fluttervibecode.dpdns.org/pricing for plan details.`
      : `Hey @${authorUsername}! You've used your free generation. Upgrade your plan at fluttervibecode.dpdns.org/pricing to build more apps from tweets.`
