...
/**
 * /sign-up — premium auth page (mirror of /sign-in with name field + autoSignIn).
 */
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((res as any)?.error) {
        ...
        // autoSignIn:true in Better Auth config means we now have a session.
        router.push(next)
        router.refresh()
      }
    } catch (err: any) {
  ...
  return (
    <div className="grid min-h-[100dvh] grid-cols-1 lg:grid-cols-[1.05fr_1fr] bg-background">
      <div className="relative hidden lg:flex flex-col justify-between overflow-hidden border-r border-border/60 bg-gradient-to-br from-card/40 via-background to-background p-12">
        <div aria-hidden className="pointer-events-none absolute -top-32 -left-32 h-[420px] w-[420px] rounded-full bg-gradient-to-br from-orange-500/15 via-amber-500/8 to-transparent blur-3xl" />
        <div aria-hidden className="pointer-events-none absolute -bottom-40 right-[-10%] h-[480px] w-[480px] rounded-full bg-gradient-to-tr from-emerald-500/12 via-teal-500/6 to-transparent blur-3xl" />

        <div className="relative">
          <Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold tracking-tight text-foreground hover:text-foreground/80 transition-colors">
        ...
        <div className="relative space-y-8">
          <div className="space-y-3">
            ...
            <p className="max-w-[48ch] text-base leading-relaxed text-muted-foreground">
              Free to try. Bring your own keys for any Anthropic-compatible provider (or use ours).
            </p>
          </div>
      ...
      <div className="flex flex-col items-center justify-center px-6 py-10 sm:px-12">
        <div className="w-full max-w-[400px]">
          <Link href="/" className="lg:hidden mb-8 inline-flex items-center gap-2 text-sm font-semibold tracking-tight text-foreground">
          ...
          <div className="mb-7">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Already have one?{' '}
              <Link href={`/sign-in${next !== '/' ? `?next=${encodeURIComponent(next)}` : ''}`} className="font-medium text-foreground underline-offset-4 hover:underline">
                Sign in
              </Link>
            </p>
          </div>
          ...
          <form onSubmit={submit} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="name" className="text-xs">Name <span className="text-muted-foreground/60">(optional)</span></Label>
              <Input
                id="name"
                value={name}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="password" className="text-xs">Password <span className="text-muted-foreground/60">(min 8 chars)</span></Label>
              <div className="relative">
                <Input
                  id="password"
          ...
          <p className="mt-6 text-[11px] text-muted-foreground/70 leading-relaxed">
            ...
            <Link href="/terms-of-service" className="underline-offset-4 hover:underline">Terms of Service</Link>{' '}and{' '}
