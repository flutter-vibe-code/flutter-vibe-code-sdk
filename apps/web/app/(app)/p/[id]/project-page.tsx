...
import { FlutterModal } from '@/components/flutter-modal'
...
import { SandboxExpiryBanner } from '@/components/sandbox-expiry-banner'
...
export function ProjectPageInternal({ opencodeEnabled = false, template: templateOverride }: { opencodeEnabled?: boolean; template?: 'flutter' | 'flutter' }) {
  ...
  // Ref so useChat onResponse can call setMessages (which is itself only
  // available AFTER the useChat() call returns — classic chicken-and-egg).
  ...
  const {
    ...
    body: chatBody,
    onResponse: (res) => {
      console.log('[FVC-DEBUG] useChat onResponse:', { status: res.status, headers: Object.fromEntries(res.headers.entries()) })
    },
    onError: (err) => {
      console.error('[FVC-DEBUG] useChat onError:', err)
    },
    onFinish: (msg) => {
      console.log('[FVC-DEBUG] useChat onFinish:', { role: msg.role, contentSize: msg.content?.length, contentPreview: msg.content?.substring(0, 300) })
    },
    ...
    onResponse: (response) => {
      ...
      if (response.ok) {
        ...
        // GOAL FIX: append a preliminary assistant message to the messages array
        // so heartbeats coming as text-delta chunks have a target message to
        // accumulate into. Without this, useChat buffers chunks for several
        // seconds before creating the assistant message, and the GeneratingApp
        // placeholder (which depends on messages.some(m=>m.role==='assistant'))
        // stays visible the entire agent turn.
        try {
          ...
          console.warn('[Chat] preliminary assistant append failed:', e)
        }
      }
    },
    experimental_prepareRequestBody: ({ messages, requestData, requestBody }) => {
      ...
      const newBody = {
        ...
        // Prefer the model picked in the ProviderSelector (vibe_model_${providerId})
        // over the legacy useClaudeModel value carried in chatBody. localStorage is
        // read at submit-time so a freshly-changed selection takes effect immediately.
        claudeModel: (typeof window !== 'undefined'
          ? (() => {
              const pid = localStorage.getItem('vibe_provider_id') || 'anthropic'
              return localStorage.getItem(`vibe_model_${pid}`) || chatBody.claudeModel
            })()
          : chatBody.claudeModel),
        ...
        moonshotKey: typeof window !== 'undefined'
          ...
          : undefined,
        providerId: typeof window !== 'undefined'
          ? (localStorage.getItem('vibe_provider_id') || undefined)
          : undefined,
        effortLevel: typeof window !== 'undefined'
          ? (localStorage.getItem('vibe_effort_level') || undefined)
          : undefined,
        byokKeys: typeof window !== 'undefined'
          ? (() => { try { return JSON.parse(localStorage.getItem('byok_keys') || '{}') } catch { return undefined } })()
          : undefined,
  ...
  // Wire ref so onResponse (defined inside useChat config) can call setMessages
  // even though setMessages is only available after useChat returns.
  ...
  // Note: result?.e2bUrl is used as fallback since it's set immediately when server starts,
  // while currentProject?.e2bUrl comes from database and may not be updated yet
  ...
  const {
    ...
    projectId: projectId || null,
    userId: session?.user?.id || null,
    e2bUrl: (result as any)?.e2bUrl || currentProject?.e2bUrl || null,
    ...
    tunnelMode: (result as any)?.tunnelMode || 'e2b-public',
    ...
    onBackupServerReady: (newSandboxUrl, newE2bUrl) => {
  ...
  // Check for Flutter errors when the agent finishes working
  ...
  useEffect(() => {
    ...
    if (streamStatus === 'ready' && (prev === 'streaming' || prev === 'submitted')) {
      console.log('[Project] Agent finished, checking for Flutter errors...')
  ...
  // Track if user has seen Flutter modal
  const [hasSeenFlutterModal, setHasSeenFlutterModal] = useState(false)
  const [showFlutterModal, setShowFlutterModal] = useState(false)

  // Load Flutter modal state from localStorage
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const seen = localStorage.getItem('hasSeenFlutterModal')
      setHasSeenFlutterModal(seen === 'true')
    }
  }, [])
  ...
  const handleNativeAppClick = () => {
    ...
    if (!hasSeenFlutterModal) {
      setShowFlutterModal(true)
      setHasSeenFlutterModal(true)
      localStorage.setItem('hasSeenFlutterModal', 'true')
    } else {
      // If modal has been seen, open Flutter directly
      if ((result as any)?.e2bUrl) {
        const expoUrl = (result as any).e2bUrl.replace('https://', 'exp://')
  ...
  useEffect(() => {
    const verifySandboxAndStartServer = async () => {
      console.log('[DEBUG] verifySandboxAndStartServer called with:', {
        ...
        isRemixedProject,
        hasE2bUrl: !!currentProject?.e2bUrl,
      ...
      // For remixed projects, still check if Flutter server is running
      // but skip the full sandbox verification since remix API already handled container creation
      if (isRemixedProject) {
        console.log('[Server Check] Remixed project - checking only Flutter server status')

        if (currentProject.e2bUrl && currentProject.sandboxUrl) {
          try {
            ...
            if (!expoResult.isAlive) {
              console.log('[Server Check] Flutter server is down on remixed project, checking if sandbox still exists...')
              ...
              try {
                ...
                if (!sandboxCheck.isAlive) {
                  ...
                  // Sandbox exists, just restart the Flutter server
                  console.log('[Server Check] Sandbox exists, restarting Flutter server...')
            ...
            console.error('[Server Check] Error checking Flutter server for remixed project:', error)
          }
        }
      ...
      if (isInitialLoad.current && isNewProject) {
        console.log('[Server Check] Initial project creation - starting Flutter server directly')
      ...
      try {
        ...
        // Sandbox is running - now verify Flutter server is also running
        if (currentProject.e2bUrl && currentProject.sandboxUrl) {
          console.log('[Server Check] Sandbox running and URLs exist, verifying Flutter server...')
          console.log('[Server Check] Current e2bUrl:', currentProject.e2bUrl)
          console.log('[Server Check] Current sandboxUrl:', currentProject.sandboxUrl)

          // Check if Flutter server is actually responding
          ...
          try {
            ...
            if (!expoCheckResponse.ok) {
              ...
              if (expoServerIsAlive) {
                console.log('[Server Check] ✅ Both sandbox and Flutter server are running, showing preview')
                setResult({
            ...
            // Flutter server is down, need to restart it
            console.log('[Server Check] ⚠️ Sandbox running but Flutter server is down (isAlive:', expoServerIsAlive, '), restarting server...')
          } catch (expoError) {
            console.error('[Server Check] Error checking Flutter server, will restart:', expoError)
          }
        } else {
          console.log('[Server Check] No URLs in database, will start server from scratch')
        }

        // Either no URLs or Flutter server is down - need to start/restart the server
        console.log('[Server Check] Starting/restarting Flutter server...')
        isStartingServerRef.current = true
        setIsPreviewLoading(true)
    ...
    const handleVisibilityChange = async () => {
      ...
      visibilityDebounceRef.current = setTimeout(async () => {
        ...
        try {
          ...
          // STEP 2: Sandbox is running — check Flutter server only if not in recovery cooldown
          if (currentProject.e2bUrl && !isInRecoveryCooldown()) {
            try {
              const expoCheckResponse = await fetch('/api/check-expo-server', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
              ...
              if (expoResult.isAlive) {
                console.log('[Visibility] Sandbox and Flutter server healthy, no action needed')
                return
              }

              console.log('[Visibility] Flutter server not responding, restarting...')
              isStartingServerRef.current = true
              setIsPreviewLoading(true)
              setIsSandboxRecovering(true)
              try {
                ...
                console.error('[Visibility] Error restarting Flutter server:', error)
              } finally {
                setIsPreviewLoading(false)
                setIsSandboxRecovering(false)
                isStartingServerRef.current = false
              ...
              console.log('[Visibility] Error checking Flutter server:', error)
            }
          }
        } catch (error) {
  ...
  const checkServerStatus = async () => {
    ...
    try {
      ...
      if (serverResult.success) {
        ...
        // Update result with server URL and e2bUrl
        const newResult = {
          url: serverResult.url,
          e2bUrl: serverResult.e2bUrl,
          tunnelMode: serverResult.tunnelMode,
      ...
      if (response.ok) {
        ...
        if (isRemixedProject) {
          ...
          console.log('[Remix] Project data:', {
            ...
            e2bUrl: data.project.e2bUrl,
            title: data.project.title
          })

          // Set the result with URLs that are already in the database
          if (data.project.sandboxUrl || data.project.e2bUrl) {
  ...
  const createNewProject = async () => {
    console.log('Creating new project with:', {
      ...
      template: templateOverride || templateFromUrl || selectedTemplate || 'flutter',
      userID: session?.user?.id,
    })
    ...
    try {
      ...
      if (result.success) {
        ...
        setResult({
          ...
          template: templateOverride || project.template || 'flutter',
  ...
  const handlePreviewRefresh = useCallback((urls: { url: string; e2bUrl?: string }) => {
...
<button
      ...
      {false && process.env.NODE_ENV === 'development' && (
        ...
        />
        <FlutterModal
          open={showFlutterModal}
          onOpenChange={setShowFlutterModal}
        />
        <SubscriptionModal
        ...
        {/* Floating pill showing remaining sandbox lifetime + Extend button. */}
        <SandboxExpiryBanner
          projectId={projectId}
          userID={session?.user?.id}
        />
