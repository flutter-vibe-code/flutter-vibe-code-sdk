...
export async function POST(request: Request) {
  try {
    ...
    const e2bUrl = `https://${ngrokDomain}.ngrok.dev`

    switch (action) {
      case 'start_backup':
        if (tunnelMode === 'lan') {
          return await restartNgrokOnly(sandbox, sandboxId, projectId, ngrokDomain, e2bUrl)
        }
        return await startBackupServer(sandbox, sandboxId, projectId, ngrokDomain, e2bUrl)

      case 'cleanup_and_restart':
        if (tunnelMode === 'lan') {
          return await restartNgrokOnly(sandbox, sandboxId, projectId, ngrokDomain, e2bUrl)
        }
        return await cleanupAndRestart(sandbox, sandboxId, projectId, ngrokDomain, e2bUrl)
...
async function startBackupServer(
  ...
  try {
    ...
    if (ngrokWorking) {
      console.log('[ngrok-backup-server] Testing ngrok URL:', e2bUrl)
      try {
        const response = await fetch(e2bUrl, {
          method: 'GET',
