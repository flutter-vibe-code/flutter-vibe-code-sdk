...
import { db, projects, user, eq } from '@flutter-vibe-code/database'
...
import { getPusherServer } from '@flutter-vibe-code/pusher/server'
import { canUserCreateSandbox } from '@flutter-vibe-code/byok'
...
export const maxDuration = 3600 // 1 hour — matches E2B sandbox lifetime on hobby/free plan
...
export async function POST(req: Request) {
  ...
  // BYOK = any explicit key in legacy fields OR any value in the per-provider byokKeys record
  const hasByokKey =
    !!anthropicKey ||
    (agentType === 'kimi-k2' && !!moonshotKey) ||
    (!!byokKeys && Object.values(byokKeys).some(v => !!v))
  // Skip the rate-limit gate for admin emails so privileged users do not hit Choose-Your-Plan.
  let userIsAdmin = false
  if (userId) {
    try {
      const u = await db.select({ email: user.email }).from(user).where(eq(user.id, userId)).limit(1)
      if (u.length > 0 && isAdmin(u[0].email)) userIsAdmin = true
    } catch (err) { console.warn("[Chat Route] admin email lookup failed", err) }
  }
  if (userId && lastUserMessage && !hasByokKey && !userIsAdmin) {
  ...
  // Increment message usage count before processing (skip for BYOK users + admins)
  if (!hasByokKey && !userIsAdmin) {
