  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          // Emit an immediate heartbeat so the chat replaces 'Generating your app' placeholder
          // with real progress within ~200ms (otherwise the user stares at the placeholder
          // for 10-15s while the sandbox boots before generateAppStreaming even starts).
          try {
            const hb = JSON.stringify({
              type: 'message',
              content: JSON.stringify({ type: 'assistant', subtype: 'text', text: '⏳ Connecting to sandbox…' }),
              timestamp: new Date().toISOString(),
            })
            ...
            sandbox,
            {
              ...
              onComplete: async (result: any) => {
                ...
                const finalResult = {
                  ...
                  template: project!.template || 'flutter',
