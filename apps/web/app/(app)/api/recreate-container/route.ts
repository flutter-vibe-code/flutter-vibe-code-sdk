...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // Only Flutter is supported now; legacy templates collapse to flutter.
      const templateSelection: keyof typeof templateId = 'flutter'
...
`
    ...
    // Start Expo server for Flutter projects (both production and testing templates)
    if (project.template === 'flutter') {
      try {
        const currentTunnelMode = await tunnelModeFlag()
        const serverResult = await startFlutterServer(sandbox, project.id, undefined, currentTunnelMode as any)
        return Response.json({
          success: true,
          projectId: project.id,
