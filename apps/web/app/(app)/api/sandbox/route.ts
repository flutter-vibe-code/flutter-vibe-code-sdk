...
// Generate project title from fragment code or description using AI
async function generateProjectTitle(fragment: FragmentSchema): Promise<string> {
  ...
  if (fragment.code && typeof fragment.code === 'string') {
    ...
    // Try to use the first meaningful line as a title
    for (const line of lines) {
      const trimmed = line.trim()
      if (trimmed.length > 5 && !trimmed.startsWith('//') && !trimmed.startsWith('import')) {
        const words = trimmed.split(/\s+/).slice(0, 3).join(' ')
        if (words.length > 3) {
          return words.toLowerCase().replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, '-').substring(0, 40)
        }
      }
    }
  }
...
function getTemplateFilePath(
  ...
  switch (template) {
    case 'flutter':
      // For Flutter Expo, ensure files are in the correct app directory
    ...
    default:
      // For other templates, default to app directory
      return `/home/user/app/${aiGeneratedPath.replace(/^\.\//, '')}`
  }
}
...
export async function POST(req: Request) {
  ...
  if (!sbx) {
    const templateIdMap: Record<string, string> = {
      'flutter': 'sce6zutko6kvhyhza4n8',
      'flutter': 'wxe2y93k4kafhbwqg2br',
    }
    ...
    if (isFirstMessage || !project) {
      // Generate title - try AI generator first
      let title = `${fragment.template.replace(/[-_]/g, ' ').replace(/^\w/, (c) => c.toUpperCase())} Project`
      try {
        const { generateTitleFromUserMessage } = await import('@/lib/name-generator')
        const firstMessage = { role: 'user', content: fragment.userMessage || fragment.description || '' }
        if (firstMessage.content) {
          title = await generateTitleFromUserMessage({ message: firstMessage })
          console.log('[Sandbox] AI-generated title:', title)
        }
      } catch (err) {
        console.log('[Sandbox] AI title generation failed, using fallback:', err)
      }
  ...
  // Handle Flutter Expo specific setup (both production and testing templates)
  if (fragment.template === 'flutter') {
    ...
    // Start Flutter web server with hot reload
    console.log('Starting Flutter web server...')

    let serverOutput = ''
    let flutterReady = false

    // For Flutter, we need to run `flutter run -d web-server` on port 3000
    // This provides hot restart for real-time updates
    const flutterStartCmd = [
      'cd /home/user/app',
      'rm -f /tmp/flutter_dev.log',
      'nohup flutter run -d web-server --web-hostname 0.0.0.0 --web-port 3000 > /tmp/flutter_dev.log 2>&1 &',
      'echo "Flutter started with PID $!"',
      'disown',
    ].join(' && ')
    
    console.log('[Sandbox] Flutter command:', flutterStartCmd)
    
    // Start Flutter in background mode
    sbx.commands
      .run(flutterStartCmd, {
    ...
    // For Flutter, server runs on port 3000, for Expo on 8081
    const flutterPort = 3000
    const expoPort = 8081
    const publicHostFlutter = sbx.getHost(flutterPort)
    const publicHostExpo = sbx.getHost(expoPort)
    
    // Use the correct host based on which server is running
    // For Flutter template, use port 3000; for Expo, use 8081
    publicUrl = `https://${publicHostFlutter}?sandboxId=${sbx.sandboxId}`

    console.log('E2B public URL (Flutter):', publicUrl)
