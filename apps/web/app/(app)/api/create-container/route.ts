...
import { projects, convexProjectCredentials, projectEnvVars } from '@flutter-vibe-code/database'
...
import { updateAppConfigWithName } from '@flutter-vibe-code/publish'
...
import { recordSandboxSession } from '@flutter-vibe-code/byok'
import { startFlutterServer } from '@/lib/server-utils'
...
export async function POST(req: NextRequest) {
  try {
      ...
      if (existingProjects.length > 0) {
        ...
        if (project.sandboxId) {
          try {
            ...
            try {
              const { updateSandboxEnvFile } = await import('@/lib/convex/sandbox-utils')
              const apiBaseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://fluttervibecode.dpdns.org'
    ...
    // Prioritize chooseTemplate from request body, fallback to env var, then default to 'flutter'
    ...
    try {
      ...
      if (error.status === 404) {
        // Create the repository if it doesn't exist. Auto-detect whether the
        // configured GITHUB_OWNER is an org or a user — `createInOrg` 404s for
        // personal accounts (which is how every project written by the agent
        // got lost: repo never created → no backup → sandbox TTL killed it).
        try {
          console.log(`Creating GitHub repository ${owner}/${repositoryName}`)
          let isOrg = false
          try {
            const ownerInfo = await octokit.users.getByUsername({ username: owner })
            isOrg = ownerInfo.data.type === 'Organization'
          } catch (probeErr) {
            console.warn(`[Create Container] Could not probe owner type for ${owner}, assuming user`, probeErr)
          }
          if (isOrg) {
          ...
          console.log(`[Create Container] Repository ${owner}/${repositoryName} created (${isOrg ? 'org' : 'user'} mode)`)
    ...
    // Boot Flutter dev server (mirrors recreate-container/route.ts).
    // Without this, the sandbox spawns but nothing serves on the preview port —
    // the frontend hits "Connection refused" forever.
    if (project.template === 'flutter') {
      try {
        const currentTunnelMode = await tunnelModeFlag()
        const serverResult = await startFlutterServer(
          sandbox,
          project.id,
          undefined,
          currentTunnelMode as any,
        )
        return Response.json({
          success: true,
          sandboxId: sandbox.sandboxId,
        ...
        console.error('[Create Container] Failed to start Flutter server:', error)
        // Still return success on container — frontend can poll /api/server-status
        return Response.json({
          success: true,
          sandboxId: sandbox.sandboxId,
          projectId: project.id,
          ...
          isNew: true,
          serverStartError:
            error instanceof Error ? error.message : 'Unknown error starting Flutter',
        })
      }
    }
