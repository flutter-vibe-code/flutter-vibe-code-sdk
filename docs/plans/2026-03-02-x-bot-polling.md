...
import { xBotState } from '@flutter-vibe-code/database'
...
async function getAuthClient(): Promise<Client> {
  ...
  const oauth2Client = new auth.OAuth2User({
    ...
    callback: 'https://flutter-vibe-code.dev/api/x-bot/auth/callback',
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://flutter-vibe-code.dev'
