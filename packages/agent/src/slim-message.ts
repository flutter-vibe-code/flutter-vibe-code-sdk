...
export interface SlimToolResult {
  ...
  subtype: 'tool_result'
  success: boolean
  /** Truncated tool output / error message — what the IDE shows when user expands the Done card. */
  output?: string
}
...
export function slimifyMessage(message: SDKMessage): SlimMessage[] {
  switch (message.type) {
    ...
    case 'user': {
      // Tool results — headline + truncated output so the chat can show what
      // actually happened when the user expands the Done / Tool Error card.
      const msg = message as any
      const contentBlocks: any[] = msg.message?.content ?? msg.content ?? []
      let success = true
      ...
      for (const block of contentBlocks) {
        ...
        // Collect text payloads from any shape the SDK uses
        if (typeof block.content === 'string') {
          if (block.content.startsWith('Error:')) success = false
          outputChunks.push(block.content)
        } else if (Array.isArray(block.content)) {
          for (const c of block.content) {
            if (typeof c === 'string') outputChunks.push(c)
            else if (c && typeof c.text === 'string') outputChunks.push(c.text)
          }
      ...
      const MAX = 1000
      const output = joined.length > MAX
        ? joined.slice(0, MAX) + '\n…[truncated ' + (joined.length - MAX) + ' chars]'
        : joined
