...
# OpenCode CLI removed: the IDE now exclusively uses Claude Agent SDK
# (multi-provider routed via Anthropic-compat baseURL). No need to install
# the opencode-ai binary anymore — saves ~30s build time and ~80MB.
...
# better-sqlite3 (omniroute dep) needs python3 + gcc/g++ to compile natively via node-gyp
RUN apt-get update \
