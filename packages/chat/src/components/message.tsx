...
import { cn } from "@flutter-vibe-code/ui/lib/utils";
import { MessageContent } from "./message-content";
import { Button } from "@flutter-vibe-code/ui/components/button";
...
const PureChatMessage = ({
  ...
  if (typeof window !== "undefined") {
    console.log("[FVC-DEBUG] PureChatMessage render", {
      id: message.id,
      role: message.role,
  ...
  const isStillGenerating = !hasAssistantResponse && (isLoading || status === "submitted");
  // Hide the card as soon as streaming starts so heartbeats / agent output become visible.
  // Previously this also waited on status === "streaming", which kept the card up for the
  // full agent turn whenever the text-delta buffer had not yet created an assistant message.
  const shouldShowGeneratingCard = isPreparingApp && message.role === "user" && (status === "submitted" || !hasAssistantResponse && isLoading);
